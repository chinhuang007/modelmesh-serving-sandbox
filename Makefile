...
fvt:
	go test -v ./fvt -ginkgo.v -ginkgo.progress -test.timeout 60m

# Command to regenerate the grpc go files from the proto files
fvt-protoc:
	rm -rf fvt/generated
	protoc -I=fvt/proto --go_out=plugins=grpc:. --go_opt=module=wmlserving.ai.ibm.com/controller $(shell find fvt/proto -iname "*.proto")

fvt-with-deploy: oc-login deploy-release fvt
